# -*- coding: utf-8 -*-
"""
Created on Sun Jan  6 22:07:13 2019
...
name = []
for f in files:
    image_files.append(path[12:] + "\\" + f)

for f in files:
    name.append(f[:-4])    

test_image_files = np.array(image_files[:10000])
name = name[:10000]


#trains_validate_files, test_files, trains_validate_targets, test_targets = \
    train_test_split(image_files, image_targets, test_size=0.20, random_state=42)

#train_files, valid_files, train_targets, valid_targets = \
#    train_test_split(trains_validate_files, trains_validate_targets, test_size=0.02, random_state=42)
...
test_files = image_files  # name of the files full path
...
test_tensors = paths_to_tensor(test_image_files).astype('float32')/255
...
final_dataset["id"] = name
final_dataset["category"] = predictions

final_dataset["category"]+=1
